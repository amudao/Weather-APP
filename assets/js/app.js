...
async function getData(city) {
    ...
    let req = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=71587bf95ccd447aabe130949212309&q=${city}&days=1&aqi=no&alerts=no`);
    ...
    if (req.ok) {
        ...
        let day = document.getElementById("dayy");
        let date  = document.getElementById("datee");
        let location = document.getElementById("locationn");
        let temp = document.getElementById("tempp");
        ...
        temp.innerHTML = `${allData.current.temp_c}<sup>o</sup>C`;
        status.innerHTML = `${allData.current.condition.text}`;
